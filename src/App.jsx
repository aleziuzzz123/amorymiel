...
function App() {
  ...
  const trackEvent = async (eventType, eventData = {}) => {
    ...
    try {
      ...
      // Track in Facebook Conversions API (Server-side)
      try {
        let facebookEventName = FACEBOOK_EVENTS.PAGE_VIEW;
        let facebookEventData = {};
        
        // Map event types to Facebook events
        switch (eventType) {
          case 'purchase':
            facebookEventName = FACEBOOK_EVENTS.PURCHASE;
            facebookEventData = {
              value: eventData.orderTotal || 0,
              currency: 'MXN',
              productName: eventData.productName,
              productIds: eventData.productIds || [eventData.productId],
              email: user?.email,
            ...
            break;
          case 'add_to_cart':
            facebookEventName = FACEBOOK_EVENTS.ADD_TO_CART;
            facebookEventData = {
              value: eventData.productPrice || 0,
              currency: 'MXN',
              productName: eventData.productName,
              productIds: [eventData.productId],
            ...
            break;
          case 'page_view':
            facebookEventName = FACEBOOK_EVENTS.PAGE_VIEW;
            facebookEventData = {
              email: user?.email,
              firstName: user?.displayName?.split(' ')[0],
              lastName: user?.displayName?.split(' ').slice(1).join(' '),
            ...
            break;
          case 'product_view':
            facebookEventName = FACEBOOK_EVENTS.VIEW_CONTENT;
            facebookEventData = {
              productName: eventData.productName,
              productIds: [eventData.productId],
              category: eventData.category,
            ...
            break;
          case 'search':
            facebookEventName = FACEBOOK_EVENTS.SEARCH;
            facebookEventData = {
          ...
          case 'add_to_wishlist':
            facebookEventName = FACEBOOK_EVENTS.ADD_TO_WISHLIST;
            facebookEventData = {
              productName: eventData.productName,
              productIds: [eventData.productId],
              category: eventData.category,
        ...
        // Create and send Facebook Conversion Event
        const facebookEvent = createFacebookEvent(facebookEventName, facebookEventData);
        await sendFacebookConversionEvent(facebookEvent);
        
      } catch (conversionError) {
        console.error('Error sending Facebook Conversion Event:', conversionError);
        // Don't throw error to avoid breaking the main tracking
      }
    } catch (error) {
